...
module.exports.crearUsuarioDemo = function(callback){ //Función para crear usuarios
  var arr = [{ email: 'demo@gmail.com', user: "demo", password: "demo"}, { email: 'andres@mail.com', user: "andres", password: "demo"}]; //array con la información de los usuarios a insertar
  Usuario.insertMany(arr, function(error, docs) { //Utilizar la función insertMany para insertar varios registros en una sola consulta
    if (error){ //Acciones si existe un error
      if (error.code == 11000){ //Verificar si el nombre de usuario (PrimaryKey) del existe
        callback("Utilice los siguientes datos: </br>usuario: demo@ | password:demo </br>usuario: juan | password:123456") //Mostrar mensaje
      }else{
        callback(error.message) //Mostrar mensaje de error
      }
    }else{
      callback(null, "El usuario 'demo' y 'andres' se ha registrado correctamente. </br>usuario: demo | password:demo </br >usuario: juan | password:demo") //Mostrar mensaje del usuario guardado con exito
    }
  });
}
